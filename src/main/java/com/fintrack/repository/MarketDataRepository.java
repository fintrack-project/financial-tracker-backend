...
public interface MarketDataRepository extends JpaRepository<MarketData, UUID> {
    @Query(value="SELECT m.id, m.symbol, m.price, m.percent_change, m.timestamp, m.asset_name, m.price_unit FROM market_data m WHERE m.asset_name IN (:assetNames)", nativeQuery = true)
